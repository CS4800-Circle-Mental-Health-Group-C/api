...
public class UserFacade {
  ...
  public UserSurveyResponse deleteUserSurvey(String userId, String surveyId) {
    Survey survey = surveyService.deleteUserSurvey(userId, surveyId);
    return UserSurveyResponse.from(survey);
  }
  ...
  public UserHealthScoreResponse getUserHealthScore(String id) {
    User user = userService.findById(id);

    return UserHealthScoreResponse.from(user);
  }

  public UserHealthScoreResponse updateUserHealthScore(String id, int healthScore) {
    User user = userService.findById(id);
    user.setHealthScore(healthScore);
    User updatedUser = userService.updateUser(user);

    return UserHealthScoreResponse.from(updatedUser);
  }
}
